...
// We are defining the longest path (including filenames) we will test with:
extern const unsigned int MAX_PATH_LENGTH;

/*
 * Data structure for storing information about a single process.
 * Each tree node contains contains a link to its first child (the next level
...
// Function for printing the TreeNodes encountered on a preorder traversal of
// a PTree to a specified maximum depth.
